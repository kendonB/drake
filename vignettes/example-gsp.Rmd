...
If you wish to tinker with the example yourself, you can generate the [underlying code files](https://github.com/wlandau-lilly/drake/tree/master/inst/examples/gsp) with `drake_example("gsp")` and then step through `run.R`.
...
At this point, we can gather together the whole workflow plan. Because `drake` handles the sequence in which things run, these can be in any order you like.
...
We also need the [report.Rmd file](https://github.com/wlandau-lilly/drake/blob/master/inst/examples/gsp/report.Rmd) for this example.
